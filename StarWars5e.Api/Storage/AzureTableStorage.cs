...
namespace StarWars5e.Api.Storage
{
    internal class AzureTableStorage : IAzureTableStorage
    {
        ...
        private readonly bool _checkIfTablesExist;

        public AzureTableStorage(string connectionString, bool checkIfTablesExist = false)
        {
            var account = CloudStorageAccount.Parse(connectionString);
            _client = account.CreateCloudTableClient();

            _tables = new ConcurrentDictionary<string, CloudTable>();
            _checkIfTablesExist = checkIfTablesExist;
        }
